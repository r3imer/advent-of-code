...
public static class Task2 {
    ...
    public static string run() {
        ...
        var output = input
            ...
            .Select(x => { 
                var list = digits
                    .Select(r => r.Matches(x).Select(x => (x.Index,x.Value)))
                    .SelectMany(x => x)
                    .OrderBy(x => x.Index)
                    ;
                return 
                    list.First().Value.convert() * 10 + 
                    list.Last().Value.convert();
            })
            // .Select(x => string.Join(",", x.Select(x => x.Value)))
            // .Select(x => x.First().convert() * 10 + x.Last().convert())
            .Sum()
            .ToString()
            ; 
        return string.Join('\n', output);
    ...
    static int convert(this string x) {
        return x switch {
            "one" => 1,
            "two" => 2,
            "three" => 3,
